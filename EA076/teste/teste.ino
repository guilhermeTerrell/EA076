...
unsigned char paginaEEPROM = 0x07;/*7*/
unsigned char posicaoMemoriaEEPROM = 0xff;/*255*/
unsigned char penultimaPosicaoMemEEPROM = 0x01;
unsigned char ultimaPosicaoMemEEPROM = 0x00;
...
bool leituraHabilitada = 0;/*flag que habilita novo ciclo de leitura*/
...
unsigned int delayLeitura = 0;/*Especifica o tempo que deve ser agaurdado entre leituras na EEPROM*/
...
void escreveLCD(String funcao) {
  ...
  if(funcao == "Transferência de dados"){
    lcd.setCursor(0, 0);
    lcd.print("tranferindo...");
  }
}
...
void atualizaEEPROM(){
  escreveEEPROM(enderecoEEPROM, 7, 254, paginaEEPROM);
  escreveEEPROM(enderecoEEPROM, 7, 255, posicaoMemoriaEEPROM);
}
...
float leTemp(){
  ...
  temp = (5000*valorAnalogLM35) / (1024*10);/*Temperatura final do LM35*/
  //Serial.println(temp);
  return temp;
}
...
void statusEEPROM(){
  unsigned char paginasOcupadas = 0;
  unsigned char posicoesOcupadas = 0;
  paginasOcupadas = leEEPROM(enderecoEEPROM, 7, 254);
  posicoesOcupadas = leEEPROM(enderecoEEPROM, 7, 255);
  memOcupada = (paginasOcupadas << 8) + posicoesOcupadas;
...
void transfDadosEEPROM(unsigned int qdadeDados){
  if(qdadeDados <= 1020){
    if((((pagina << 8) + posicao) <= (2*qdadeDados) - 1)){
      if(posicao % 2 == 0){
...
void loop(){
  //maqEstados(funcao);
  //escreveLCD(funcao);
  unsigned char paginasOcupadas = 123;
  unsigned char posicoesOcupadas = 123;
  //paginasOcupadas = leEEPROM(enderecoEEPROM, 7, 254);
  posicoesOcupadas = leEEPROM(enderecoEEPROM, 7, 255);
  Serial.println(paginasOcupadas);
  Serial.println(posicoesOcupadas);
  while(1);
}
